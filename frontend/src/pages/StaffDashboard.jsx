...
export function StaffDashboard() {
  ...
  const [stats, setStats] = useState([
    { label: "Pending Tickets", value: 0, icon: Clock, color: "text-amber-600", bg: "bg-amber-50" },
    { label: "Escalated Tickets", value: 0, icon: AlertCircle, color: "text-red-600", bg: "bg-red-50" },
  ]);
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        setStats([
          { label: "Pending Tickets", value: pendingCount, icon: Clock, color: "text-amber-600", bg: "bg-amber-50" },
          { label: "Escalated Tickets", value: escalatedCount, icon: AlertCircle, color: "text-red-600", bg: "bg-red-50" },
        ]);
        setTickets(recent);
      } catch (error) {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Staff Dashboard</h1>
        <p className="text-sm text-slate-500 mt-1">Overview of your assigned tickets and SLA performance.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {loading ? (
          ...
          stats.map((stat) => (
            <div key={stat.label} className="bg-white border border-slate-200 rounded-sm p-6 shadow-sm flex items-center gap-4">
      ...
      <div className="bg-white border border-slate-200 rounded-sm shadow-sm overflow-hidden flex flex-col">
        <div className="px-6 py-4 border-b border-slate-200 bg-white">
          <h2 className="text-base font-semibold text-slate-900">Recent Assigned Tickets</h2>
        </div>

        {loading ? (
          <div className="p-6 space-y-4">
            {Array.from({ length: 3 }).map((_, i) => (
              <Skeleton key={`t-skel-${i}`} className="h-12 w-full rounded-sm bg-slate-50" />
            ))}
          </div>
        ) : tickets.length > 0 ? (
            ...
            <TableBody className="divide-y divide-slate-100">
              {tickets.map((ticket) => (
                ...
                >
                  ...
                  <TableCell className="text-slate-600">{ticket.student}</TableCell>
                  <TableCell className="truncate max-w-[200px]">{ticket.subject}</TableCell>
                  <TableCell>
                    <span className={`text-xs font-semibold flex items-center gap-1.5 ${ticket.isOverdue ? 'text-red-600' : 'text-slate-600'}`}>
          ...
          <div className="p-8">
            <EmptyState
              ...
              description="You have no tickets physically assigned to your queue at this moment."
            />
          </div>
        )}
      </div>
